...
## 7. Local build workflow (this fork)

This fork adds a two-step local build process for faster iteration.
...
This clones vanilla OpenWrt 24.10, the MediaTek feeds, applies custom patches and packages from `my_files/`, then runs the MTK autobuild framework (prepare + build). Images go to `openwrt/bin/targets/mediatek/filogic/`.
...
After the first successful build, `mypatch.sh` is faster — it only:

1. Overwrites device tree files (`my_files/mt7988a*.dtsi`)
2. Copies the kernel config (`my_files/.config`)
3. Clones `fantastic-packages` (extra packages)
4. Runs `autobuild.sh build` (skip prepare)

You can also go into `openwrt/` and run `make V=s -j$(nproc)` directly.

### Custom files

| Path | Purpose |
|---|---|
| `my_files/mt7988a.dtsi` | Custom SoC device tree (MT7988A) |
| `my_files/mt7988a-bananapi-bpi-r4.dtsi` | Custom board DTS — SFP1: mod-def0-gpios removed for XGS-PON sticks |
| `my_files/.config` | Kernel / OpenWrt config |
| `my_files/my_final_defconfig` | Package selection defconfig |
| `my_files/*.patch` | Additional kernel patches |
| `mypatch.sh` | Post-first-build rebuild script |

### SFP note

SFP1 on BPI-R4 uses GPIO82 for MOD_DEF0 (module detect). XGS-PON sticks like Leolabs LXE-010S-H have an internal Linux that toggles MOD_DEF0 during boot, causing a false "module removed" event. This fork removes `mod-def0-gpios` from DTS — the kernel polls the module via I2C with retries instead.
